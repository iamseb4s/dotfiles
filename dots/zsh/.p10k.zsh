# Generated by Powerlevel10k configuration wizard on 2024-02-11 at 16:29 -05.
...
typeset -g POWERLEVEL9K_PROMPT_CHAR_OK_{VIINS,VICMD,VIVIS,VIOWR}_FOREGROUND='#61afef'
...
if (( $1 )); then
    ...
    local       meta='%F{#808080}'  # grey foreground
    ...
    local       meta='%F{#808080}' # grey foreground
    local      clean='%F{#808080}'  # grey foreground
    local   modified='%F{#808080}'  # grey foreground
    local  untracked='%F{#808080}'  # grey foreground
    local conflicted='%F{#808080}'  # grey foreground
fi
...
if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
    ...
    # res+="${branch_color}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}" # Original line
    # Next two lines show different icons for local and remote branches.
    local my_icon="${${VCS_STATUS_REMOTE_NAME:+  }:- }"
    res+="${branch_color}${my_icon}${branch//\%/%%}"
fi
...
typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND='#eeeeee'
...
typeset -g POWERLEVEL9K_BACKGROUND_JOBS_RIGHT_SEGMENT_SEPARATOR=
...
typeset -g POWERLEVEL9K_BATTERY_{CHARGING,CHARGED}_FOREGROUND=32
...
#####################################[ wifi: wifi speed ]#####################################
# WiFi color.
typeset -g POWERLEVEL9K_WIFI_FOREGROUND=178
...
typeset -g POWERLEVEL9K_TIME_RIGHT_PROMPT_FIRST_SEGMENT_START_SYMBOL='\uE0B6'


#########################[ example: user-defined prompt segment example ]#########################
